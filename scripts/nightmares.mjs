...
} from './nightmare-model.mjs';

const DOMAIN = 'nightmares';
const rolledSaves = new Set();
const FLAGS = (document) => document?.flags?.[ID] ?? {};
const enabled = () =>
  game.settings.get(ID, 'config')?.enabled !== false && readState(DOMAIN).enabled !== false;
const randomId = () =>
  globalThis.foundry?.utils?.randomID?.() ?? crypto.randomUUID().replaceAll('-', '');
const outcomes = () =>
  Object.fromEntries(
    ['criticalSuccess', 'success', 'failure', 'criticalFailure'].map((key) => [
      key,
...
const fail = (error) => {
  console.error(`${ID} | nightmare`, error);
  globalThis.ui?.notifications?.error(error.message ?? String(error));
};
const officialChapter = () => {
  try {
    return latestUnlockedChapter(game.settings.get(OFFICIAL, 'campaign'));
  } catch {
    return null;
  }
};
...
export function buildPhobiaData(source, session) {
  ...
  data.system.traits = {
    ...data.system.traits,
    value: [...new Set([...(data.system.traits?.value ?? []), 'curse'])]
  };
  if (
    session.chapter === 1 &&
    !data.system.rules.some((r) => r.key === 'RollOption' && r.option === 'lake-laroba-in-sight')
  ) {
    data.system.rules.push({
      key: 'RollOption',
      option: 'lake-laroba-in-sight',
      label: t('Nightmare.DeepWaterVisible'),
      toggleable: true,
      value: false
    });
  }
  data.flags = {
    ...
    [ID]: {
      ...data.flags?.[ID],
      nightmare: { kind: 'phobia', sessionId: session.id, chapter: session.chapter }
    }
  };
  return markLocalized(data, {
    name: `Nightmare.Phobia${session.chapter}Name`,
    description: phobiaDescription(session.chapter),
    rules:
      session.chapter === 1
        ? [
            {
              index: data.system.rules.findIndex(
                (r) => r.key === 'RollOption' && r.option === 'lake-laroba-in-sight'
              ),
              label: 'Nightmare.DeepWaterVisible'
            }
          ]
        : []
  });
...
export function buildUneaseData(session) {
  const data = {
    name: t('Nightmare.UneaseName'),
    type: 'effect',
    img: 'icons/magic/control/fear-fright-monster-grin-red-orange.webp',
    system: {
      slug: 'bob-nightmare-unease',
      description: { value: t('Nightmare.UneaseDescription'), gm: '' },
      rules: [],
      duration: { value: 24, unit: 'hours', expiry: 'turn-start', sustained: false },
      start: { value: session.at, initiative: null },
      traits: { value: ['curse'], rarity: 'common' }
    },
    flags: { [ID]: { nightmare: { kind: 'unease', sessionId: session.id } } }
  };
  return markLocalized(data, {
    name: 'Nightmare.UneaseName',
    description: 'Nightmare.UneaseDescription'
  });
}
    ...
    if (requirement === 'flat-check') {
      ...
      if (flatTotal === undefined) {
        ...
        await updateState(DOMAIN, (state) => {
          state.sessions[id].flatTotal = flatTotal;
        });
        await roll.toMessage({ flavor: t('Nightmare.RestCheckFlavor') }, { rollMode: 'gmroll' });
      }
    }
    const state = await updateState(DOMAIN, (draft) => {
      confirmRest(draft, { id, actorIds, chapter, flatTotal });
    });
...
function remainingDuration(data, until) {
  ...
  data.system.duration = {
    ...data.system.duration,
    value: Math.max(0, (until - time) / 60),
    unit: 'minutes'
  };
...
export async function settleNightmareResult({ id, actorId, outcome }) {
  return withAction(DOMAIN, async () => {
    ...
    if (result.until > now()) {
      if (
        result.applyPhobia &&
        !Array.from(actor.items).some(
          (item) =>
            FLAGS(item).nightmare?.kind === 'phobia' && FLAGS(item).nightmare.sessionId === id
        )
      ) {
...
export async function requestNightmareSave({ id, actorId }) {
  return withAction(DOMAIN, async () => {
    ...
    if (
      !session.nightmare ||
      !session.actorIds.includes(actorId) ||
      session.results?.[actorId] ||
      session.suggestions?.[actorId]
    )
      throw new Error(t('Nightmare.NoPendingSave'));
    const existing = Array.from(game.messages ?? []).find(
      (m) =>
        FLAGS(m).nightmareRequest?.sessionId === id && FLAGS(m).nightmareRequest.actorId === actorId
    );
...
export async function requestNightmareSaves({ id }) {
  ...
  const unresolved = session.actorIds.filter(
    (actorId) => !session.results?.[actorId] && !session.suggestions?.[actorId]
  );
  const messages = [];
...
export async function rollNightmareSave({ id, actorId, dc }) {
  ...
  if (typeof actor.saves?.will?.roll !== 'function')
    throw new Error(t('Nightmare.NativeWillUnavailable'));
...
export async function consumeNightmareFrightened({ actorId, actionId }) {
  return withAction(DOMAIN, async () => {
    ...
    if (!condition || condition.slug !== 'frightened')
      throw new Error(t('Nightmare.FrightenedChanged'));
    if (FLAGS(condition).nightmareFrightened?.actionId !== action.id) {
      ...
      await condition.update({
        'system.value.value': action.target,
        [`flags.${ID}.nightmareFrightened`]: { actionId: action.id, target: action.target }
      });
    }
    const markers = Array.from(actor.items).filter(
      (item) =>
        FLAGS(item).nightmare?.kind === 'unease' &&
        FLAGS(item).nightmare.sessionId === action.sessionId
    );
    if (markers.length) {
      requirePrimaryGM();
...
function needsExpiry(state, at) {
  return (
    Object.values(state.characters ?? {}).some(
      (pc) => pc.pending && !pc.pending.consumed && !pc.pending.expired && at >= pc.pending.until
    ) ||
    Object.values(state.sessions ?? {}).some((s) =>
      Object.values(s.results ?? {}).some(
        (r) => r.status === 'complete' && !r.expired && at >= r.until
      )
    )
  );
}
...
async function expireEffects(at) {
  ...
  for (const [actorId, sessions] of elapsed) {
    ...
    const ids = Array.from(actor?.items ?? [])
      .filter((item) => sessions.has(FLAGS(item).nightmare?.sessionId))
      .map((item) => item.id);
    if (ids.length) {
      requirePrimaryGM();
...
function getForm(dialog) {
  return new FormData(dialog.element.querySelector('form'));
}
const option = (value, text, selected = false) =>
  `<option value="${esc(value)}" ${selected ? 'selected' : ''}>${esc(text)}</option>`;
const formRow = (label, content) => `
  <label class="bob-field"><span>${label}</span>${content}</label>`;
const notice = (html) => `<p class="bob-hint">${html}</p>`;
const readableTime = (value) =>
  Number.isFinite(value) ? formatTime(value) : t('Nightmare.UnknownTime');

async function setupNightmares() {
  requirePrimaryGM();
  const chapter = officialChapter();
  const result = await foundry.applications.api.DialogV2.wait({
    window: { title: t('Nightmare.SetupTitle') },
    position: { width: 540 },
    classes: ['bob-companion'],
    content:
      notice(t('Nightmare.SetupHint')) +
      (chapter
        ? `<p>${t('Nightmare.CurrentChapter', { chapter })}</p>`
        : formRow(
            t('Nightmare.ChapterUnavailable'),
            `<select name="chapter">${Object.keys(NIGHTMARE_CHAPTERS)
              .map((ch) => option(ch, t('Nightmare.Chapter', { chapter: ch })))
              .join('')}</select>`
          )) +
    ...
    buttons: [
      {
        action: 'save',
        label: t('Nightmare.SaveSetup'),
        default: true,
        callback: (_e, _b, d) => ({
          chapter: chapter ?? Number(getForm(d).get('chapter')),
          alreadyHandled: getForm(d).get('baseline') === 'handled'
        })
      },
      { action: 'cancel', label: t('Nightmare.Cancel') }
    ],
    rejectClose: false
  });
  if (!result || result === 'cancel') return;
  await updateState(DOMAIN, (draft) => {
    initializeNightmares(draft, { ...result, at: now() });
  });
}
...
async function editRest(id) {
  ...
  const available = new Map(
    [...partyMembers(), ...record.actorIds.map((id) => game.actors.get(id)).filter(Boolean)].map(
      (a) => [a.id, a]
    )
  );
  const requirement = chapter ? restRequirement(readState(DOMAIN), chapter) : null;
  const result = await foundry.applications.api.DialogV2.wait({
    window: { title: t('Nightmare.ConfirmSleepTitle') },
    position: { width: 580 },
    classes: ['bob-companion'],
    content:
      notice(t('Nightmare.ConfirmSleepHint')) +
      ...
      (requirement
        ? notice(
            t(requirement === 'flat-check' ? 'Nightmare.FlatCheckHint' : 'Nightmare.FirstNightHint')
          )
        : '') +
      [...available.values()]
        .map(
          (actor) =>
            `<label style="display:block"><input type="checkbox" name="sleepers" value="${esc(actor.id)}" ${record.actorIds.includes(actor.id) ? 'checked' : ''}> ${esc(actor.name)}</label>`
        )
        .join(''),
    buttons: [
      {
        action: 'confirm',
        label: t('Nightmare.ConfirmSleep'),
        default: true,
        callback: (_e, _b, d) => ({
          actorIds: getForm(d).getAll('sleepers'),
          chapter: chapter ?? Number(getForm(d).get('chapter'))
        })
      },
      { action: 'ignore', label: t('Nightmare.NotApplicable') },
      { action: 'cancel', label: t('Nightmare.Later') }
    ],
    rejectClose: false
  });
  if (result === 'ignore') {
...
async function showResults(id) {
  const record = sessionFor(id);
  const unresolved = record.actorIds.filter(
    (actorId) => !record.results?.[actorId] && !record.suggestions?.[actorId]
  );
  const rows = record.actorIds
    .map((actorId) => {
      const actor = game.actors.get(actorId),
        result = record.results?.[actorId],
        suggested = record.suggestions?.[actorId]?.outcome;
      return `<div class="bob-card"><h3>${esc(actor?.name ?? actorId)}</h3>${
        ...
        <select aria-label="${esc(t('Nightmare.SaveOutcome'))}" name="outcome-${esc(actorId)}">${Object.entries(
          outcomes()
        )
          .map(([key, label]) =>
            option(key, label, key === (result?.outcome ?? suggested ?? 'success'))
          )
          .join('')}</select>
        <button type="button" data-action="settle" data-actor="${esc(actorId)}">${t(result ? 'Nightmare.RetrySettlement' : 'Nightmare.ConfirmOutcome')}</button>`
      }</div>`;
    })
    .join('');
  class ResultsDialog extends foundry.applications.api.DialogV2 {
    _onRender(context, options) {
      ...
      this.element
        .querySelector('[data-action="request-all"]')
        ?.addEventListener('click', async (event) => {
          const button = event.currentTarget;
          if (button.disabled) return;
          button.disabled = true;
          try {
            await requestNightmareSaves({ id });
          } catch (error) {
            fail(error);
          } finally {
      ...
      this.element.querySelectorAll('[data-action][data-actor]').forEach((button) =>
        button.addEventListener('click', async () => {
          ...
          try {
            ...
            if (button.dataset.action === 'roll') await rollNightmareSave({ id, actorId });
              ...
              await settleNightmareResult({
                id,
                actorId,
                outcome: getForm(this).get(`outcome-${actorId}`)
              });
              await this.close();
  ...
  return ResultsDialog.wait({
    window: { title: t('Nightmare.ResultsTitle') },
    position: { width: 650 },
    classes: ['bob-companion'],
    ...
    buttons: [{ action: 'close', label: t('Nightmare.Close') }],
    rejectClose: false
  });
}
...
async function correctionDialog() {
  ...
  const result = await foundry.applications.api.DialogV2.wait({
    window: { title: t('Nightmare.CorrectionTitle') },
    classes: ['bob-companion'],
    position: { width: 550 },
    content:
      notice(t('Nightmare.CorrectionHint')) +
      formRow(
        t('Nightmare.Actor'),
        `<select name="actor">${partyMembers()
          .map((a) => option(a.id, a.name))
          .join('')}</select>`
      ) +
      formRow(
        t('Nightmare.ChapterLabel'),
        `<select name="chapter">${Object.keys(NIGHTMARE_CHAPTERS)
          .filter((ch) => Number(ch) <= chapter)
          .map((ch) => option(ch, t('Nightmare.Chapter', { chapter: ch }), Number(ch) === chapter))
          .join('')}</select>`
      ) +
    ...
    buttons: [
      {
        action: 'save',
        label: t('Nightmare.SaveCorrection'),
        callback: (_e, _b, d) => Object.fromEntries(getForm(d))
      },
      { action: 'cancel', label: t('Nightmare.Cancel') }
    ],
    rejectClose: false
  ...
  await updateState(DOMAIN, (draft) => {
    correctCharacter(draft, {
      actorId: result.actor,
      chapter: Number(result.chapter),
      phobiaHandled: result.phobia === 'on',
      clearPending: result.clear === 'on',
      at: now()
    });
    if (result.first === 'on') {
      draft.firstNightHandled = true;
      for (let ch = 1; ch <= chapter; ch++) (draft.chaptersHandled ??= {})[ch] = true;
    }
  });
  if (result.clear === 'on') {
    const actor = actorFor(result.actor),
      ids = Array.from(actor.items)
        .filter((i) => FLAGS(i).nightmare?.kind === 'unease')
        .map((i) => i.id);
    if (ids.length) {
      requirePrimaryGM();
      await actor.deleteEmbeddedDocuments('Item', ids);
...
export async function openNightmares() {
  ...
  const restRow = (session, actionable = false) => {
    const names = session.actorIds.map((id) => game.actors.get(id)?.name ?? id).join(', ');
    const statusKey = actionable
      ? session.status === 'pending'
        ? 'Nightmare.PendingSleep'
        : 'Nightmare.PendingResults'
      : session.nightmare
        ? 'Nightmare.ResultsComplete'
        : 'Nightmare.NoNightmareShort';
    return `
      <li class="bob-record">
        <div>
          <strong>${esc(names)}</strong>
          <p class="hint">${esc(readableTime(session.at))}</p>
          <p>${t(statusKey)}</p>
        </div>
        ${
          actionable && primary
            ? `
          <button type="button" class="bob-primary" data-action="rest" data-id="${esc(session.id)}">
            ${t('Nightmare.Handle')}
          </button>`
            : ''
        }
  ...
  const currentStatus = actors
    .map((actor) => {
      const pc = state.characters?.[actor.id];
      const curse = pendingFrightened(state, actor.id, now());
      const recovering = pc?.frightenedAction?.status === 'applying';
      if (!curse && !recovering) return '';
      return `
      <li class="bob-record">
        <div>
          <strong>${esc(actor.name)}</strong>
          <p>${curse ? t('Nightmare.PendingFear', { duration: formatDuration(curse.until - now()) }) : t('Nightmare.FearRecovery')}</p>
        </div>
        ${
          primary
            ? `
          <button type="button" data-action="fear" data-actor="${esc(actor.id)}">
            ${t(recovering ? 'Nightmare.RetryFear' : 'Nightmare.ApplyFirstFear')}
          </button>`
            : ''
        }
      </li>`;
    ...
    .join('');
  const body = `
    ${!primary ? notice(t('Nightmare.ReadOnlyStatus')) : ''}
    ${state.enabled === false ? `<p class="bob-warning">${t('Nightmare.AutomationPaused')}</p>` : ''}
    ${
      !state.initialized
        ? `
      <div class="bob-section">
        <p>${t('Nightmare.SetupNeeded')}</p>
        ${primary ? `<button type="button" class="bob-primary" data-action="setup">${t('Nightmare.Setup')}</button>` : ''}
      </div>`
        : ''
    }
    <section class="bob-section" aria-labelledby="bob-rests-heading-${viewId}">
      <h3 id="bob-rests-heading-${viewId}">${t('Nightmare.PendingRests')}</h3>
      ${
        pending.length
          ? `<ul class="bob-record-list">${pending.map((session) => restRow(session, true)).join('')}</ul>`
          : `<p class="bob-empty">${t('Nightmare.NoPendingRests')}</p>`
      }
    </section>
    <section class="bob-section" aria-labelledby="bob-effects-heading-${viewId}">
      <h3 id="bob-effects-heading-${viewId}">${t('Nightmare.CurrentStatus')}</h3>
      ${
        currentStatus
          ? `<ul class="bob-record-list">${currentStatus}</ul>`
          : `<p class="bob-empty">${t('Nightmare.NoActiveEffects')}</p>`
      }
    </section>
    ${
      recent.length
        ? `
      <section class="bob-section" aria-labelledby="bob-recent-rests-heading-${viewId}">
        <h3 id="bob-recent-rests-heading-${viewId}">${t('Nightmare.RecentResults')}</h3>
        <ul class="bob-record-list">${recent.map((session) => restRow(session)).join('')}</ul>
      </section>`
        : ''
    }
    <details>
      <summary>${t('Nightmare.Advanced')}</summary>
      <p>${chapter ? t('Nightmare.CurrentChapter', { chapter }) : t('Nightmare.ChapterUnavailable')}</p>
      ${state.initialized ? `<p class="hint">${t(state.firstNightHandled ? 'Nightmare.PastNightsHandled' : 'Nightmare.FirstNightPending')}</p>` : ''}
      ${
        primary
          ? `
        <div class="bob-toolbar">
          ${
            state.initialized
              ? `
            <button type="button" data-action="manual">${t('Nightmare.ManualRest')}</button>
            <button type="button" data-action="correct">${t('Nightmare.CorrectRecords')}</button>`
              : ''
          }
          <button type="button" data-action="toggle">
            ${t(state.enabled === false ? 'Nightmare.ResumeAutomation' : 'Nightmare.PauseAutomation')}
          </button>
        </div>`
          : ''
      }
  ...
  class NightmaresDialog extends foundry.applications.api.DialogV2 {
    _onRender(context, options) {
      ...
      this.element
        .querySelectorAll(
          '[data-action="setup"], [data-action="toggle"], [data-action="correct"], [data-action="manual"], [data-action="rest"], [data-action="fear"]'
        )
        .forEach((button) =>
          button.addEventListener('click', async () => {
            if (button.disabled) return;
            ...
            try {
              ...
              switch (button.dataset.action) {
                ...
                case 'manual': {
                  ...
                  await updateState(DOMAIN, (s) => {
                    queueRest(s, { id, actorIds: actors.map((a) => a.id), at: now() });
                  });
                  await editRest(id);
                  break;
                }
                case 'rest':
                  await editRest(button.dataset.id);
                  break;
                case 'fear': {
                  const recovering =
                    readState(DOMAIN).characters?.[button.dataset.actor]?.frightenedAction
                      ?.status === 'applying';
                  if (
                    recovering ||
                    (await foundry.applications.api.DialogV2.confirm({
                      window: { title: t('Nightmare.ConfirmFirstFearTitle') },
                      content: `<p>${t('Nightmare.ConfirmFirstFearHint')}</p>`
                    }))
                  )
                    await consumeNightmareFrightened({ actorId: button.dataset.actor });
                  break;
                }
                default:
                  return;
  ...
  return NightmaresDialog.wait({
    window: { title: t('Nightmare.HomeTitle') },
    position: { width: 700 },
    classes: ['bob-companion'],
    content: body,
    buttons: [{ action: 'close', label: t('Nightmare.Close') }],
...
function queueNativeMessage(message, options) {
  ...
  event.actorIds.push(actorId);
  event.messageIds.push(message.id);
  arrivals.set(id, event);
  clearTimeout(arrivalTimer);
  arrivalTimer = setTimeout(() => {
    const batch = [...arrivals.values()];
    arrivals.clear();
...
async function captureSave(message) {
  ...
  const marker = Array.from(context.options ?? []).find(
    (o) => typeof o === 'string' && o.startsWith('bob-nightmare:')
  );
  if (!marker) return;
  const id = marker.slice('bob-nightmare:'.length),
    actorId = message.speaker?.actor;
  const record = readState(DOMAIN).sessions?.[id];
  const canRecord = (current) => {
    if (!current?.nightmare || !current.actorIds.includes(actorId) || current.results?.[actorId])
      return false;
...
export function registerNightmares() {
  ...
  Hooks.on('preCreateChatMessage', (message, _data, options) => {
    ...
    if (!batch) {
      batch = { id: `rest-${randomId()}`, at: now() };
      nativeBatches.set(options, batch);
    }
