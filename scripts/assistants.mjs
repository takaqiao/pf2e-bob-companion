...
const entries = [
  ['openNightmares', 'Hub.Rest'],
  ['openBoons', 'Hub.Boons'],
  ['openHazards', 'Hub.Hazards'],
  ['openSoulhearts', 'Hub.Soulhearts']
];
const macros = [
  ...
  return withAction('assistants:macros', async () => {
    for (const [key, method, img] of macros) {
      ...
      const data = {
        ...
        command,
        ownership: { default: 0 },
        flags: { [ID]: { assistantMethod: method } }
      };
...
export function openAssistants() {
  ...
  opening = showAssistants().finally(() => {
    opening = null;
    activeWindow = null;
  });
  return opening;
    ...
    content: `
      <nav class="bob-tool-list" aria-label="${esc(t('Hub.Title'))}">
        ${entries
          .map(
            ([method, label]) => `
          <button type="button" class="bob-tool-link" data-assistant="${method}">
            ${esc(t(label))}
          </button>
        `
          )
      ...
      <details>
        <summary>${t('Hub.Shortcuts')}</summary>
        <div class="bob-toolbar">
          <button type="button" data-assistant="installAssistantMacros">${t('Hub.InstallMacros')}</button>
        </div>
      </details>`,
...
export function registerAssistants() {
  class Menu extends foundry.applications.api.ApplicationV2 {
    render() {
      void openAssistants().catch((error) => ui.notifications.error(error.message));
      return this;
    }
  }
  game.settings.registerMenu(ID, 'assistants', {
    name: 'BOB.Hub.Menu',
    label: 'BOB.Hub.Open',
    hint: 'BOB.Hub.Hint',
  ...
  Hooks.once('ready', () =>
    Object.assign((game.modules.get(ID).api ??= {}), { openAssistants, installAssistantMacros })
  );
}
