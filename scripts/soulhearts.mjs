...
const actorById=id=>{const actor=game.actors.get(id);if(!actor)throw new Error(t('Soulheart.ErrorActorMissing'));return actor;};
const ownedItem=(actor,id)=>{const item=actor.items.get(id);if(!item)throw new Error(t('Soulheart.ErrorItemMissing'));return item;};
...
export function findHPEffect(actor) {
  ...
  if(effects.length>1)throw new Error(t('Soulheart.ErrorMultipleHPEffects',{actor:actor.name??actor.id}));
  const effect=effects[0];if(!effect)return null;
  if(!hpRule(effect)||!Number.isInteger(Number(effect.system.badge?.value)))throw new Error(t('Soulheart.ErrorHPEffectUnreadable'));
...
function hpData(template,value,id) {
  ...
  return markLocalized(data,{name:'Soulheart.HPEffectName',description:'Soulheart.HPEffectDescription',values:{value}});
}
export function lifeEffectData(template,op) {
  ...
  return markLocalized(data,{name:'Soulheart.LifeEffectName',description:'Soulheart.LifeEffectDescription',values:{value:op.temporaryHP}});
}
...
export function makeSoulheartPort({read=()=>readState(DOMAIN),save=mutator=>updateState(DOMAIN,mutator),actor=actorById,load=loadEffect,assertGM=requirePrimaryGM,time=now,environment=getEnvironment}={}) {
  ...
  const changed=()=>{throw new Error(t('Soulheart.ErrorStale'));};
  ...
  return {read,save,
    async preflight(op) {
      ...
      if(op.kind!=='sync') {
        ...
        if(typeof actor(op.phantomId).toggleRollOption!=='function')throw new Error(t('Soulheart.ErrorRankInterface'));
      }
      for(const pc of op.beneficiaries) {
    ...
    async preflightLife(op) {
      ...
      if(flag(item).soulheartDaily && time()<flag(item).soulheartDaily.resetAt)throw new Error(t('Soulheart.ErrorDailyUsed'));
    ...
    async applyLife(op,step) {
      ...
      if(time()>=op.expiresAt) {
        await save(state=>{state.operations[op.id].notice='Soulheart.NoticeExpired';});return;
      }
    ...
    async apply(op,step,undo=false) {
      ...
      if(step==='rank') {
        ...
        if(phantomRank(phantom).rank!==after)throw new Error(t('Soulheart.ErrorRankNotUpdated'));
        return;
      }
      const pc=op.beneficiaries.find(pc=>`hp:${pc.id}`===step);if(!pc)throw new Error(t('Soulheart.ErrorHPStep'));
      ...
      assertGM();
      if(effect)await effect.update({name:data.name,'system.description':data.system.description,'system.badge':data.system.badge,[`flags.${ID}.soulheartHP`]:true,[`flags.${ID}.lastUpgrade`]:marker,[`flags.${ID}.localization`]:data.flags[ID].localization});
...
const gradeLabel=grade=>tr({ordinary:'GradeOrdinary',greater:'GradeGreater',major:'GradeMajor'}[grade]);
const select=(name,label,entries,selected)=>`<label class="bob-field"><span>${esc(label)}</span><select name="${esc(name)}">${entries.map(([value,text])=>`<option value="${esc(value)}" ${value===selected?'selected':''}>${esc(text)}</option>`).join('')}</select></label>`;
const partyChoices=()=>partyMembers().filter(actor=>actor.type==='character');
const recipientsHTML=()=>`<fieldset><legend>${esc(tr('Beneficiaries'))}</legend>${partyChoices().map(actor=>`<label class="bob-field"><input type="checkbox" name="beneficiary" value="${esc(actor.id)}" checked> ${esc(actor.name)}</label>`).join('')}</fieldset>`;
const dialog=()=>foundry.applications.api.DialogV2;
export const soulheartEligibleItems=(holder,state={},mode='upgrade')=>list(holder?.items).filter(item=>Number(item.system?.quantity)>0&&(mode==='bind'?soulheartGrade(item,'ordinary'):soulheartGrade(item,state.bindings?.[item.uuid])));
export const soulheartEligibleHolders=(actors,state={},mode='upgrade')=>list(actors).filter(actor=>soulheartEligibleItems(actor,state,mode).length);
...
function summary(op) {
  if(op.kind==='life')return tr('LifePreview',{holder:actorById(op.holderId).name,item:ownedItem(actorById(op.holderId),op.itemId).name,phase:tr(op.phase==='day'?'Day':'Night'),hp:op.temporaryHP});
  return tr('UpgradePreview',{item:ownedItem(actorById(op.holderId),op.itemId).name,grade:gradeLabel(op.grade),before:op.quantityBefore,after:op.quantityAfter,phantom:actorById(op.phantomId).name,rankBefore:op.rankBefore,rankAfter:op.rankAfter,totalBefore:op.totalBefore,totalAfter:op.totalAfter});
}
...
export async function soulheartOperationDialog(mode='upgrade') {
  requirePrimaryGM();const state=readState(DOMAIN),holders=holdersFor(state,mode);
  if(!holders.length)throw new Error(tr('ErrorNoHolder'));
  const preferred=preferredActor(holders),initial=holders.includes(preferred)?preferred:holders[0];
  const itemEntries=itemsFor(initial,state,mode).map(item=>[item.id,`${item.name} × ${item.system.quantity}`]);
  const content=`${select('holder',tr('Holder'),holders.map(actor=>[actor.id,actor.name]),initial.id)}${select('item',tr('Item'),itemEntries)}${mode==='upgrade'?`${select('phantom',tr('Phantom'),[])}${recipientsHTML()}`:''}<div data-soulheart-preview aria-live="polite"></div>`;
  let shown=null,signature=null;
  const formSignature=form=>{const data=selectedForm(form);return JSON.stringify({holder:data.get('holder'),item:data.get('item'),phantom:data.get('phantom'),beneficiaries:data.getAll('beneficiary')});};
  const refresh=(form,changed)=>{
    const holder=holders.find(actor=>actor.id===form.elements.namedItem('holder')?.value),itemSelect=form.elements.namedItem('item');
    if(changed==='holder')itemSelect.innerHTML=options(itemsFor(holder,state,mode).map(item=>[item.id,`${item.name} × ${item.system.quantity}`]));
    const item=itemsFor(holder,state,mode).find(row=>row.id===itemSelect.value),phantomSelect=form.elements.namedItem('phantom');
    if(phantomSelect) {
      const grade=item&&soulheartGrade(item,state.bindings?.[item.uuid]);
      const eligible=grade?list(game.actors).filter(actor=>{try{return phantomRank(actor).rank===SOULHEART_GRADES[grade].rank;}catch{return false;}}):[];
      const old=phantomSelect.value;phantomSelect.innerHTML=options(eligible.map(actor=>[actor.id,actor.name]));
      if(changed!=='item'&&eligible.some(actor=>actor.id===old))phantomSelect.value=old;
    }
    ...
    try {
      ...
      shown=mode==='life'?previewSoulheartLife({holderId:holder.id,itemId:item.id}):previewSoulheart({holderId:holder.id,itemId:item.id,phantomId:data.get('phantom'),beneficiaryIds:data.getAll('beneficiary')});
      const html=`<p>${esc(summary(shown))}</p>${beneficiaryPreviewHTML(shown)}`;
      signature=formSignature(form);preview.innerHTML=html;
    } catch(error) {shown=null;signature=null;preview.textContent=error.message;}
    const submit=form.closest('.application')?.querySelector('button[data-action="apply"]');if(submit)submit.disabled=!shown;
  };
  const result=await dialog().wait({window:{title:`BoB｜${tr(mode==='life'?'LifeTitle':'UpgradeTitle')}`},position:{width:600},classes:['bob-companion'],content,
    render:(_event,instance)=>{const form=instance.element.querySelector('form');form.addEventListener('change',event=>refresh(form,event.target.name));refresh(form,'holder');},
    buttons:[{action:'apply',label:tr('Confirm'),default:true,callback:(_event,_button,instance)=>displayedProposal(shown,signature,formSignature(instance.element.querySelector('form')))}],rejectClose:false});
  if(result) return mode==='life'?activateSoulheartLife(result,true):commitSoulheart(result,true);
  return null;
}
async function choose(title,content,buttons=[{action:'next',label:tr('Continue')}]) {
  return dialog().wait({window:{title:`BoB｜${title}`},position:{width:600},classes:['bob-companion'],content,rejectClose:false,buttons:buttons.map(button=>({...button,callback:(_event,_button,instance)=>({action:button.action,form:selectedForm(instance.element.querySelector('form'))})}))});
}
async function chooseBind() {
  const state=readState(DOMAIN),holders=holdersFor(state,'bind');if(!holders.length)throw new Error(tr('ErrorNoEquipment'));
  const holderResult=await choose(tr('BindTitle'),select('holder',tr('Holder'),holders.map(actor=>[actor.id,actor.name])));if(!holderResult)return;
  const holder=actorById(holderResult.form.get('holder')),items=itemsFor(holder,state,'bind');
  const result=await choose(tr('BindTitle'),select('item',tr('Item'),items.map(item=>[item.id,item.name]))+select('grade',tr('Grade'),Object.keys(SOULHEART_GRADES).map(grade=>[grade,gradeLabel(grade)])));if(!result)return;
  const item=ownedItem(holder,result.form.get('item')),grade=result.form.get('grade');
  if(await dialog().confirm({window:{title:tr('BindTitle')},content:`<p>${esc(tr('BindConfirm',{holder:holder.name,item:item.name,grade:gradeLabel(grade)}))}</p>`,rejectClose:false}))await withAction(ACTION,()=>updateState(DOMAIN,state=>{state.bindings??={};state.bindings[item.uuid]=grade;}));
}
async function chooseHPSync() {
  ...
  const result=await choose(tr('SyncTitle'),`<label class="bob-field">${esc(tr('Baseline'))}<input name="baseline" type="number" min="${minimum}" value="${minimum}" step="1"></label>${recipientsHTML()}`);if(!result)return;
  const op=previewSoulheartHPSync({beneficiaryIds:result.form.getAll('beneficiary'),baseline:Number(result.form.get('baseline'))});
  if(await dialog().confirm({window:{title:tr('SyncTitle')},content:`<p>${esc(tr('SyncPreview',{before:op.totalBefore,after:op.totalAfter}))}</p>${beneficiaryPreviewHTML(op)}`,rejectClose:false}))await commitSoulheart(op,true);
}
const actorName=id=>game.actors.get(id)?.name??tr('RemovedActor');
const operationLabel=op=>op.kind==='life'?(op.notice?tr('RecordLifeAttempt',{actor:actorName(op.holderId)}):tr('RecordLife',{actor:actorName(op.holderId),hp:op.temporaryHP})):tr('RecordUpgrade',{actor:op.holderId?actorName(op.holderId):op.beneficiaries.map(pc=>actorName(pc.id)).join(', '),hp:op.totalAfter});
const noticeText=notice=>notice?.startsWith?.('Soulheart.')?t(notice):notice;
export const soulheartRecordHTML=op=>`<li>${esc(operationLabel(op))} · ${esc(formatTime(op.createdAt))}${op.notice?`<p>${esc(noticeText(op.notice))}</p>`:''}${op.error?`<p>${esc(op.error)}</p>`:''}</li>`;
export async function openSoulhearts() {
  ...
  const content=`<p>${esc(tr('Total',{hp:Number(state.total??0)}))}</p>${pending.length?`<p role="status">${esc(tr('PendingCount',{count:pending.length}))}</p>`:''}${recent.length?`<ul>${recent.map(soulheartRecordHTML).join('')}</ul>`:''}<details><summary>${esc(tr('Advanced'))}</summary><div class="bob-field"><button type="button" data-soulheart-advanced="bind">${esc(tr('BindTitle'))}</button><button type="button" data-soulheart-advanced="sync">${esc(tr('SyncTitle'))}</button>${pending.length?`<button type="button" data-soulheart-advanced="retry">${esc(tr('Retry'))}</button>`:''}${state.lastCompleted?`<button type="button" data-soulheart-advanced="correct">${esc(tr('Correct'))}</button>`:''}</div>${pending.length?`${select('pending',tr('Pending'),pending.map(op=>[op.id,`${operationLabel(op)} · ${formatTime(op.createdAt)}`]))}<ul>${pending.map(soulheartRecordHTML).join('')}</ul>`:''}</details>`;
  let advanced=null;
  const result=await dialog().wait({window:{title:`BoB｜${tr('Title')}`},position:{width:600},classes:['bob-companion'],content,rejectClose:false,
    render:(_event,instance)=>{instance.element.querySelectorAll('[data-soulheart-advanced]').forEach(button=>button.addEventListener('click',()=>{advanced={action:button.dataset.soulheartAdvanced,form:selectedForm(instance.element.querySelector('form'))};instance.close();}));},
    buttons:[{action:'upgrade',label:tr('UpgradeTitle')},{action:'life',label:tr('LifeTitle')},{action:'close',label:tr('Close')}].map(button=>({...button,callback:()=>button.action}))});
  try {
    if(result==='upgrade'||result==='life')await soulheartOperationDialog(result);
      ...
      if(advanced.action==='bind')await chooseBind();
        ...
        const op=readState(DOMAIN).operations?.[advanced.form.get('pending')];if(!op)throw new Error(tr('ErrorPendingMissing'));
        ...
        const op=readState(DOMAIN).operations?.[state.lastCompleted];if(!op)throw new Error(tr('ErrorPendingMissing'));
        if(await dialog().confirm({window:{title:tr('Correct')},content:`<p>${esc(tr('CorrectConfirm',{record:operationLabel(op),before:op.totalAfter,after:op.ledgerBefore}))}</p>`,rejectClose:false}))await correctSoulheart(op.id);
      }
    } else return;
