...
function validateAction(input) {
  if (!input.id || typeof input.id !== 'string') throw new Error(t('Boon.Error.OperationId'));
  if (!finite(input.at)) throw new Error(t('Boon.Error.WorldTime'));
}
function repeated(state,input) {
  const existing = state.actions?.[input.id];
  if (!existing) return false;
  if (existing.useId !== (input.useId ?? input.id)) throw new Error(t('Boon.Error.OperationConflict'));
  return true;
}
...
export function recordBoon(state,input) {
  ...
  if (!kinds.has(input.kind) || !input.actorId) throw new Error(t('Boon.Error.BoonAndActor'));
  if (repeated(state,input)) {
    const previous=state.uses[input.id];
    if(previous.kind!==input.kind||(previous.originalActorId??previous.actorId)!==input.actorId)throw new Error(t('Boon.Error.OperationActorConflict'));
    return previous;
  }
  if (cooldownRemaining(state,input.kind,input.actorId,input.at)) throw new Error(t('Boon.Error.Cooldown'));
  if (['A14','A19'].includes(input.kind) && (!finite(input.start) || input.at - input.start < 3600)) throw new Error(t('Boon.Error.OneHour'));
  ...
  if (input.kind === 'A14') {
    if (!['criticalSuccess','success','failure','criticalFailure'].includes(input.degree)) throw new Error(t('Boon.Error.WillResult'));
    ...
    if(input.dayStart!=null&&(!finite(input.dayStart)||input.at<input.dayStart||input.at>=input.dayStart+DAY))throw new Error(t('Boon.Error.Midnight'));
    ...
    if (!['performance','crafting','athletics'].includes(input.choice)) throw new Error(t('Boon.Error.Statue'));
    ...
    throw new Error(t('Boon.Error.AlreadyHolds'));
  }
  init(state);
...
function available(state,input) {
  ...
  if (!use || use.voided) throw new Error(t('Boon.Error.MissingOrVoided'));
  if (use.consumedAt != null) throw new Error(t('Boon.Error.Consumed'));
  if (use.expired || (finite(use.expiresAt) && input.at >= use.expiresAt)) throw new Error(t('Boon.Error.Expired'));
  return use;
}
...
export function consumeBoon(state,input) {
  ...
  if (use.kind !== 'G4' && !(use.kind === 'A14' && use.bonus > 0)) throw new Error(t('Boon.Error.NotConsumable'));
...
export function transferBoon(state,input) {
  if (repeated(state,input)) return;
  const use = available(state,input);
  if (use.kind !== 'G4' || !input.actorId || input.actorId === use.actorId) throw new Error(t('Boon.Error.NewHolder'));
  if (uses(state).some(other => other.kind === 'G4' && other.actorId === input.actorId && active(other))) throw new Error(t('Boon.Error.RecipientHolds'));
  use.transfers ??= [];
...
export function correctBoon(state,input) {
  ...
  if (repeated(state,input)) return;
  const use = state.uses?.[input.useId];
  if (!use || !input.reason?.trim()) throw new Error(t('Boon.Error.CorrectionReason'));
  if (!finite(input.start) || input.start > input.at || (input.expiresAt != null && (!finite(input.expiresAt) || input.expiresAt <= input.at))) throw new Error(t('Boon.Error.CorrectionTime'));
  if (['A14','A19'].includes(use.kind) && input.at - input.start < 3600) throw new Error(t('Boon.Error.OneHour'));
  use.corrections ??= [];
...
export function voidBoon(state,input) {
  ...
  if(repeated(state,input))return;
  const use=state.uses?.[input.useId];
  if(!use||!input.reason?.trim())throw new Error(t('Boon.Error.VoidReason'));
  if(use.voided)throw new Error(t('Boon.Error.Voided'));
